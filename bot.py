...
async def ef_add_saving(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Running total = sum of all previous Amount Saved rows (skip header only)
        prev_total = 0
...
async def inv_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Read BEFORE appending so previous snapshot index is correct
        existing_rows = ws.get_all_values()[1:]  # skip header
        asset_rows = [r for r in existing_rows if len(r) >= 4 and r[1] == wallet and r[2] == asset]

        ws.append_row([date_str, wallet, asset, value, ""])

        growth_msg = ""
        if len(asset_rows) >= 1:
            prev_val = int(str(asset_rows[-1][3]).replace(",", "").strip() or 0)
            if prev_val > 0:
...
async def show_summary(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        try:
            ws   = book.worksheet("EmergencyFund")
            rows = ws.get_all_values()[1:]  # skip header
            total = sum(int(str(r[1]).replace(",","").strip() or 0) for r in rows if len(r) > 1 and r[1])
            target = 2988000
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            MAIN_MENU: [
                CommandHandler("summary", show_summary),
